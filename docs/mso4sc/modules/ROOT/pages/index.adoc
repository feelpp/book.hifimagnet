...
** give the app a meaningfull name
...
* In your profile in the top right,  you will see an *Admin* link, click on it
* In the administrative panel, you can see and manage user
* ...
...
[[hpc_setting]]
=== Setting up an HP resource in *Experiments*
...
Before proceeding to the setup of the app, you need to make sure, at least for {hifimagnet}
 ...
 been granted the proper rigth on {hifimagnet} {sregistry} team.

One more step consists in retreiving the *TOKEN* and storing it into ypur home directory on the HPC resource.
 This *TOKEN* is needed to download programatically the {singularity} image :
...
you have choosen to use the `<hpc_volumes>` bindmount option: `${LUSTRE}/feel:/feel`.
