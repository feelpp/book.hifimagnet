...
`-t` option may be used to switch to the TUI mode.
`-b` option to the use TUI in batch mode.
...
just start the container (see <<here, qs_reference>>, watch out to not forget to enable the display
...
salome [-m GEOM,SMESH,HIFIMAGNET] [-t -b] \
   ${HIFIMAGNET}/HIFIMAGNET_Cmd.py \
   args:--cfg=HL-31.yaml[,--air[,--infty_Rratio=2,--infty_ZRatio=1.5],--mesh]
----
