...
An alternative is to build the image using the {salome} binairies, for details see link:{examplesdir}/stretch.def[stretch.def] recipe:
```
sudo -E singularity -vvv build --force --notest [--writable] "./salome-$VERSION-$TAG.simg" "./stretch.def"
```

We also provide a link:examples/build_singularity.sh[bash script] to automate the creation of {salome} singularity image:
[source,sh]
----
include::{examplesdir}/build_singularity.sh[]
----
...
The obtained singularity images may be pushed to a {sregsitry} service like link:https://sregistry.srv.cesga.es/[cesga registry]:
