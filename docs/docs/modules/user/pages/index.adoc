...
Section <7> is specific to {hifimagnet}. It enables to define
data needed to compute the "ideal" magnetic field provided by a magnet
using xref:magnettools:bmap.adoc[]:

* the cfg file describing the Axisymetrical model of the magnet
...
| <3> stem:[\sigma]   | electrical conductivity            |stem:[S.m^{-1}]           | an expression of stem:[T] with references values (indiced with a stem:[_0] at stem:[T_0] 
