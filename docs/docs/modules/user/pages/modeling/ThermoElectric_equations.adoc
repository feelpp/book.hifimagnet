...
where stem:[\textbf{j}=\sigma\textbf{E}] (from the Ohm's law).
But stem:[\exists V | \textbf{E}=-\nabla V]
...
Coefficients stem:[\sigma] and k are temperature-dependent as shown in this equations :
...
stem:[h=\frac{k(T)N_{u}}{D_{h}}] with N~u~ the Nusselt number and D~h~ the hydraulic diameter.
