...
It means that the actual simulation results are stored in `<hpc_feelpp>/hifimagnet/ThermoElectricModel/<geofile>/thermoelectric-linear_V1T1_N1/np_<parallel_tasks>` for MagThel app.
...
* the script will be created and submitted to the queueing system of the HPC ressource.

A red button will indicates an error.
In this event, check the output of the script (aka `slurm-<id>.out`). 
To do so you need to have `<do_not_delete_workdir>` set to `true`.
====
