...
[stem]
++++
\mathbf{A}(\mathbf{X}) = \mu_0 \int_{\Omega} \mathbf{j} \, G(\mathbf{X},Y) \,d\Omega_\mathbf{X} \qquad \vorall \mathbf{X} \in \mathbf{R^3} 
++++
...
[stem]
++++
	R_e &= \displaystyle \frac{\rho u}{\mu},\\
	P_r &= \displaystyle \frac{\mu C_p}{{k}, \\
